...
class CreateForm(forms.Form):
    singer = forms.CharField(label="Исполнитель", min_length=3, help_text="Введите полное название исполнителя. Например: Король и Шут")
    album = forms.CharField(label="Альбом", min_length=1, help_text="Если у песни нет альбома, то назовите альбом так же, как и песню")
    song = forms.CharField(label="Песня", min_length=1, help_text="Введите полное название песни")
...
class RegForm(forms.Form):
    username = forms.RegexField(regex="^\S*$", min_length=3, label="Логин", required=True)
